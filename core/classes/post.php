...
<div class="profile-timeline">
	<div class="news-feed-comp">
		<div class="news-feed-text">
			...
			$profileData = $this->userData($profileId);
...
//									foreach(verifiedTag as verifiedBadge){
// if(verifiedge->verified==1){
// echo '<img src="./assets/image/verify.png" style="height:15px; width:15px;" title="Only the Admin gets this Badge">';
// }
// }
...
echo $postText;
                ...
                if(empty($shareDetails)){echo 'Quote Was not found.';}else{echo '<span class="nf-2-text-span" data-postid = "'.$post->post_id.'" data-userid="'.$user_id.'" data-profilepic="'.$post->profilePic.'">'.$post->shareText.'</span>'; }
								...
								<div class="nf-pro-name-time">
									...
									<div class="nf-pro-time-privacy">
										...
										<div class="nf-pro-privacy">
											<img src="<?php BASE_URL?>/1541012386/XDA/assets/image/privacy.JPG" alt="">
										</div>
									</div>
								</div>
							</div>
...
//				Comment/Comments
                echo ''.$totalCommentCount->totalComment.' Opinions';
            } ?>
						</div>
						<div class="share-count-wrap">
							<?php if(empty($totalShareCount->totalShare)){}else{ echo ''.$totalShareCount->totalShare.' Re-quotes'; } ?>
						</div>
			...
			<div class="nf-4">
				<div class="like-action-wrap" data-postid="<?php echo $post->post_id; ?>" data-userid="<?php echo $user_id; ?>" style="position:relative;">
					...
					<div class="like-action ra">
						...
						<div class="like-action-text"><span>React</span></div>

						<?php }else{
            ?>
						<!--						If reacted, show the react icon-->
				...
				<div class="comment-action ra">
					...
					<div class="comment-action-text">
						<div class="comment-count-text-wrap">
							...
							<div class="comment-text">Opinion</div>
						</div>
					</div>
				</div>
				<div class="share-action ra" data-postid="<?php echo $post->post_id; ?>" data-userid="<?php echo $user_id ?>" data-profileid="<?php echo $profileId; ?>" data-profilePic="<?php echo $post->profilePic; ?>">
					...
					<div class="share-action-text">Quote</div>
				</div>
						...
						<li class="new-comment">
							<div class="com-details">
								<div class="com-pro-pic">
									<a href="<?php echo BASE_URL.$comment->firstName.'_'.$comment->lastName ?>">
										<span class="top-pic"><img src="<?php echo $comment->profilePic;
											?>" alt=""></span>
									</a>
								</div>
												...
												<div class="com-react-placeholder-wrap align-middle">
													<div>
														<span class="nf-pro-name">
															<a href="<?php echo BASE_URL.$comment->firstName.'_'.$comment->lastName ?>" class="nf-pro-name"><?php echo ''.$comment->firstName.' '.$comment->lastName.'';
															?></a>
															<!--															Add Badge above-->
														</span>
														<span class="com-text" style="margin-left:5px; " data-postid="<?php echo $comment->commentOn; ?> " data-userid="<?php echo $user_id; ?>" data-commentid="<?php echo $comment->commentID;  ?>" data-profilepic="<?php echo $userdata->profilePic; ?>"><?php echo $comment->comment; ?>
															<?php 
//				 $this->linkFetch();
//				 $linkFetch = $this->linkFetch($post->post_id);
//					 foreach($linkFetch as $lf){
//					 if($lf == ($comment->firstName.'_'.$comment->lastName)){
//						 echo "Hello";
//					 }
//				 }
				 #echo $linkFetch;?>
													...
													<div class="com-nf-3-wrap">
														<?php
                                                            if($com_main_react_count->maxreact == '0'){}else{
                                                                ?>
														<div class="com-nf-3 com-react-des align-middle">
															<div class="nf-3-react-icon">
																<div class="react-inst-img align-middle">
																	<?php
											...
											<div class="com-like-react" data-postid="<?php echo $comment->commentOn; ?>" data-userid="<?php echo $user_id; ?>" data-commentid="<?php echo $comment->commentID; ?>">
												...
												<?php
                                                    if(empty($commentReactCheck)){
                                                        echo '<div class="com-like-action-text"><span>React</span></div>';
                                                    }else{
														...
														<div class="com-pro-wrap">
															<div class="com-text-react-wrap">
																<div class="reply-text-option-wrap align-middle" style="justify-content: flex-start;">
																	<div class="com-pro-text align-middle">
																		<a href="">
																			<span class="nf-pro-name"><?php echo ''.$reply->firstName.' '.$reply->lastName.'' 
//																			Add Badge
																				?></span>
																		</a>
				...
				<div class="comment-write">
					...
					<div class="com-input">
						<div class="comment-input" style="flex-basis:90%;">
							<input type="text" name="" id="" class="comment-input-style comment-submit" placeholder="Inscribe your Opinion..." data-postid="<?php echo $post->post_id; ?>" data-userid="<?php echo $user_id; ?>">
						</div>
					</div>
				</div>
			</div>
...
<?php
	...
	public function verifiedTag($user_id){
		$stmt= $this->pdo->prepare('SELECT count(*) FROM `profile` WHERE `verified` = 1 AND userid = :user_id');
        $stmt->bindParam(":user_id", $user_id, PDO::PARAM_INT);

		$stmt->execute();
		$verTag=$stmt->fetch(PDO::FETCH_OBJ);
		foreach($verTag as $vt){
			if($vt == 1){
				echo '<img src="./assets/image/verify.png" style="height:15px; width:15px;" title="Only the Admin gets this Badge">';
			}
		}
//		echo (int)$stmt;
	...
	public function linkFetch(){
        $stmt = $this->pdo->prepare("SELECT u.userLink FROM users AS u LEFT JOIN comments AS c ON u.userLink = c.commentBy;");
//        $stmt->bindParam(":postid", $postid, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
		echo $stmt;
