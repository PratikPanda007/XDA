		...
		$(function() {
			...
			$('.status-share-button').on('click', function() {
				...
				if (files.length != 0) {
					...
					if (error_images == '') {
						$.ajax({
							url: 'http://localhost/1541012386/XDA/core/ajax/uploadPostImage.php',
							method: "POST",
							data: formData,
							contentType: false,
							cache: false,
			...
			function comReactSubmit(typeR, postid, userid, commentID, com_nf_3) {
				var profileid = "<?php echo $profileId; ?>";
				$.post('http://localhost/1541012386/XDA/core/ajax/commentReact.php', {
						commentid: commentID,
						reactType: typeR,
						postid: postid,
						userid: userid,
				...
				$('input.reply-input-style.reply-submit').keyup(function(e) {
					if (e.keyCode == 13) {
						...
						if (comment == '') {
							...
							$.ajax({
								type: "POST",
								url: "http://localhost/1541012386/XDA/core/ajax/reply.php",
								data: {
			...
			function replyReactSubmit(typeR, postid, userid, commentID, commentparentid, com_nf_3) {
				var profileid = <?php echo $profileId; ?>;
				$.post('http://localhost/1541012386/XDA/core/ajax/replyReact.php', {
					commentid: commentID,
					reactType: typeR,
					postid: postid,
					userid: userid,
			...
			$(document).on('click', '.post-Share', function() {
				...
				$.post('http://localhost/1541012386/XDA/core/ajax/share.php', {
					shareText: shareText,
					profileid: profileid,
					postid: postid,
					userid: userid
			...
			$(document).on('click', '.share-container', function() {
				...
				window.location.href = "http://localhost/1541012386/XDA/profile.php?username=" + userLink + "";
			})
			...
			$(document).on('keyup', 'input#main-search', function() {
				...
				if (searchText == '') {
					...
					$.post('http://localhost/1541012386/XDA/core/ajax/search.php', {
			...
			$(document).on('click', '.request-cancel', function() {
				$(this).parents('.profile-friend-confirm').removeClass().addClass('profile-add-friend').html(' <img src="assets/image/friendRequestGray.JPG" alt=""><div class="edit-profile-button-text">Add Buddy</div>');
				var userid = $(this).data('userid');
				var profileid = $(this).data('profileid');
				$.post('http://localhost/1541012386/XDA/core/ajax/request.php', {
